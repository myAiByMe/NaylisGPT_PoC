...
#
# use_graph=True  : évalue le modèle NaylisGPT avec Naylis Attention
# use_graph=False : évalue le modèle GPT classique (sans biais graphe)
#
# Install :
#   pip install lm-eval>=0.4.3
#
# Usage :
#   python bench.py --mode pretrain --model ./Model/naylis_pretrain.pt --use-graph
...
_ARCH_KEYS = [
    ...
    'use_swiglu', 'n_kv_heads', 'use_qk_norm', 'soft_cap',
    'use_flash_attn', 'rel_rank', 'use_graph',
]
...
MODEL_CFG = dict(
    ...
    use_flash_attn = True,
    dropout        = 0.0,
    use_graph      = True,   # True → Naylis Attention | False → GPT classique
)
...
def load_tokenizer(mode: str) -> AutoTokenizer:
    ...
    Note: vocab_size est détecté automatiquement depuis le checkpoint.
    """
    ...
    # Valeur de secours si le checkpoint ne contient pas vocab_size
    MODEL_CFG.setdefault("vocab_size", len(tok))
    return tok


def load_model(model_path: str, device: str, cli_use_graph=None) -> NaylisGPT:
    """
    Charge le modèle en auto-détectant sa config depuis le checkpoint.

    Priorité pour chaque clé d'architecture :
      1. model_config embedé dans le .pt  (nouveaux checkpoints)
      2. _info.json voisin du .pt         (tous les checkpoints)
      3. MODEL_CFG courant                (valeur de secours / legacy)

    cli_use_graph : None   = utilise la config du checkpoint
                   True/False = override explicite (avec avertissement si différent)
    """
    print(f"\n  Chargement : {model_path}")

    # weights_only=False car on peut avoir des objets Python dans model_config
    ...
    # use_graph : CLI override (optionnel)
    if cli_use_graph is not None and cli_use_graph != MODEL_CFG.get("use_graph"):
        print(f"  ⚠️  use_graph checkpoint={MODEL_CFG.get('use_graph')} "
              f"≠ CLI={cli_use_graph}")
        print(f"     Override CLI accepté (attention aux clés manquantes/inattendues)")
        MODEL_CFG["use_graph"] = cli_use_graph
    elif cli_use_graph is not None:
        MODEL_CFG["use_graph"] = cli_use_graph

    # ── Affichage ─────────────────────────────────────────────────────────
    graph_mode = "Naylis Attention" if MODEL_CFG["use_graph"] else "GPT classique"
    ...
    print(f"  embed={MODEL_CFG['embed_dim']}  layers={MODEL_CFG['num_layers']}  "
          ...
          f"rel_rank={MODEL_CFG['rel_rank']}")
    print(f"  use_graph={MODEL_CFG['use_graph']}  [{graph_mode}]")
...
def main():
    ...
    parser.add_argument("--device",      default="auto",
                        help="cuda / cpu / auto")
    # use_graph : doit correspondre au modèle chargé
    #   --use-graph    → NaylisGPT avec Naylis Attention (défaut)
    #   --no-use-graph → GPT classique
    g = parser.add_mutually_exclusive_group()
    g.add_argument("--use-graph",    dest="use_graph", action="store_true",  default=None,
                   help="Forcer Naylis Attention (défaut : auto depuis le checkpoint)")
    ...
    model     = load_model(args.model, device, cli_use_graph=args.use_graph)
